...
#                                 html.H4(stories[0]['explaination_var']),
...
@lru_cache()
def load_data():
    istat_data = pd.read_excel('./data/omicidi-relazione-autore-DCPC-anni-2002-2023.xlsx', sheet_name=None)
    data = DataProcessor(istat_data)
    return data.merge_df(), data
...
@lru_cache()
def load_population():
    return pd.read_csv('data/DCIS_POPRES1_10012025160913907.csv')
...
def serve_layout():
    ...
    years = load_data()[0].Year.dt.year.unique().tolist()

    return html.Div([
        html.H1(project['project']),
        html.H2(project['goal']),
        html.Div(className='container', children=[
            html.H3(project['stories'][0]['title']),
            html.H4(project['stories'][0]['statement']),
            dcc.Loading(dcc.Dropdown(years, years[-1], id='year_dropdown', className='year_dropdown')),
            dcc.Loading(dcc.Graph(id='fig_pie')),
            html.H5(project['stories'][0]['description']),
            html.Button('Reveal full story', id='reveal_button', n_clicks=0),
            html.Div(id='story1', hidden=True, children=[
                dcc.Dropdown(years, years[-1], id='year_dropdown2', className='year_dropdown'),
                dcc.Loading(dcc.Graph(id='pies_by_killer')),
                html.H4(project['stories'][0]['explaination_var']),
                html.Div(className='bar_container', children=[
                    dcc.Loading(dcc.Graph(id='bar_males')),
                    dcc.Loading(dcc.Graph(id='bar_females'))
                ]),
                html.H4(project['stories'][0]['explaination'])
            ])
        ])
    ])
...
# Callbacks
@app.callback(Output('fig_pie', 'figure'), Input('year_dropdown', 'value'))
def update_pie(year):
    data_df, _ = load_data()
    return graphs.graph_pies(data_df, year=datetime.datetime(year, 1, 1))

@app.callback(Output('pies_by_killer', 'figure'), Input('year_dropdown2', 'value'))
def update_sunburst(year):
    data_df, _ = load_data()
    return graphs.graph_sunbursts(data_df, year=year)

@app.callback(Output('story1', 'hidden'), Input('reveal_button', 'n_clicks'))
def toggle_story(n_clicks):
    return n_clicks % 2 == 0

@app.callback(Output('bar_males', 'figure'), Input('year_dropdown2', 'value'))
def update_bar_males(year):
    data_df, data = load_data()
    bar_df = data.get_bar_df(data_df, type='males')
    return graphs.graph_bars(bar_df, type='males', colors=px.colors.sequential.Blues)

@app.callback(Output('bar_females', 'figure'), Input('year_dropdown2', 'value'))
def update_bar_females(year):
    data_df, data = load_data()
    bar_df = data.get_bar_df(data_df, type='females')
    return graphs.graph_bars(bar_df, type='females', colors=px.colors.sequential.RdPu)
